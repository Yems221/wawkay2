...
class NotificationRepository @Inject constructor(
    ...
    suspend fun isDuplicateByOrangeId(
        packageName: String,
        text: String
    ): Boolean {
        // Extraire l'identifiant Orange Money (Ref:, Réf:, ou ID:)
        val orangeIdRegex = """(?:Ref|Réf|ID):\s*([A-Z0-9.]+)""".toRegex(RegexOption.IGNORE_CASE)
        val orangeMatch = orangeIdRegex.find(text)

        if (orangeMatch != null) {
            val orangeId = orangeMatch.groupValues[1].trim()
            Log.d("KUFAY_DUPLICATE", "🔍 Checking Orange Money ID: $orangeId")

            return notificationDao.checkDuplicateByOrangeId(
                packageName = packageName,
                orangeId = orangeId
            ) > 0
        }
