...
class KufayNotificationListenerService : NotificationListenerService() {
    ...
    private fun processNotification(sbn: StatusBarNotification) {
        ...
        serviceScope.launch {
            // ✨ SYSTÈME ANTI-DOUBLONS AMÉLIORÉ
            val isDuplicate = when {
                // 1️⃣ ORANGE MONEY - Vérification par ID unique (Ref/ID/CI)
                packageName == "com.google.android.apps.messaging" &&
                        title.contains("OrangeMoney", ignoreCase = true) -> {

                    // PRIORITÉ 1 : Vérifier par ID/Ref Orange Money
                    val hasDuplicateId = notificationRepository.isDuplicateByOrangeId(
                        packageName = packageName,
                        text = text
                    )

                    if (hasDuplicateId) {
                        Log.d("KUFAY_SERVICE", "🚫 DOUBLON ORANGE MONEY DÉTECTÉ (ID identique) - notification ignorée")
                        true
                    } else {
                        // PRIORITÉ 2 : Fallback par montant avec fenêtre élargie (60s)
                        amount?.let { validAmount ->
                            notificationRepository.isDuplicate(
                                packageName = packageName,
                                amount = validAmount,
                                timestamp = timestamp,
                                timeWindowMs = 60000  // 60 secondes pour SMS
                            )
                        } ?: false
                    }
                }

                // 2️⃣ MIXX BY YAS - Vérification par Ref
                packageName == "com.google.android.apps.messaging" &&
                        title.contains("Mixx by Yas", ignoreCase = true) -> {
                    ...
                    if (hasDuplicateRef) {
                        ...
                        // Fallback par montant (60s pour SMS)
                        amount?.let { validAmount ->
                            notificationRepository.isDuplicate(
                                packageName = packageName,
                                amount = validAmount,
                ...
                // 3️⃣ WAVE (Personal & Business) - Vérification par montant (5s)
                else -> {
                    amount?.let { validAmount ->
                        notificationRepository.isDuplicate(
                            packageName = packageName,
                            amount = validAmount,
                            timestamp = timestamp,
                            timeWindowMs = 5000  // 5 secondes pour Wave
                        )
                    } ?: false
                }
            }
